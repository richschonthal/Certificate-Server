...
//--------------------------------------------------------------------------------
BOOL CNTService::StartServiceThread()
	{
	AFX_MANAGE_STATE_IF_DLL

	CWinThread* pThread = ::AfxBeginThread((AFX_THREADPROC) ServiceThread, (LPVOID) this);
	if(pThread != NULL)
		{
		if(::WaitForSingleObject(pThread->m_hThread, 5000) == WAIT_OBJECT_0)
			return FALSE;
		}
...
UINT CNTService::ServiceThread(CNTService* that)
	...
	if(that->m_pMainApp != NULL)
		that->m_pMainApp->PostThreadMessage(WM_QUIT, 0, 0);
